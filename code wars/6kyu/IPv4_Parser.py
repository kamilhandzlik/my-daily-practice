...
def ipv4__parser(ip_addr, mask):
    ...
    net_parts = [ip & m for ip, m in zip(ip_parts, mask_parts)]
    host_parts = [ip & (~m & 0xFF) for ip, m in zip(ip_parts, mask_parts)]

    net_addr = '.'.join(map(str, net_parts))
    host_addr = '.'.join(map(str, host_parts))

    return net_addr, host_addr


print(ipv4__parser('192.0.2.130', '255.255.255.0'))
print(ipv4__parser('192.168.50.1', '255.255.255.0'))

# id = [f"{'0' * (8 - len(f'{int(i):2b}'))}" + f"{int(i):2b}" for i in ip_addr.split('.')]
